...
set "ROOT=%~dp0.."
set "VENV_NAME=venv_voxcpm2"
set "VENV_DIR=%ROOT%\venvs\%VENV_NAME%"
set "VENV_PY=%VENV_DIR%\Scripts\python.exe"
set "REQ=%~dp0voxcpm2_requirements.txt"

echo ============================================================
echo VoxCPM2 - Installation
echo ============================================================
echo.
...
echo [4/10] Installing PyTorch (CUDA) from: !TORCH_INDEX!
...
:torch_cpu
echo [4/10] Installing PyTorch (CPU only)...
"%VENV_PY%" -m pip install torch torchaudio --index-url https://download.pytorch.org/whl/cpu --quiet
if errorlevel 1 ( echo [ERROR] PyTorch installation failed. & pause & exit /b 1 )
...
echo.
echo [5/10] Installing voxcpm...
"%VENV_PY%" -m pip install voxcpm --quiet
if errorlevel 1 ( echo [ERROR] voxcpm installation failed. & pause & exit /b 1 )
echo [OK] voxcpm installed

echo.
echo [6/10] Installing onnxruntime...
...
:onnx_done
echo.
echo [7/10] Installing pyannote.audio...
...
echo.
echo [8/10] Installing remaining requirements from voxcpm2_requirements.txt...
if not exist "%REQ%" (
    echo [WARNING] voxcpm2_requirements.txt not found at %REQ% - skipping.
    goto :req_done
)
...
:req_done
echo.
echo [9/10] Installing FlashAttention 2 (optional, requires CUDA 12+)...
...
"%VENV_PY%" -c "from voxcpm import VoxCPM; print('[OK] voxcpm import OK')" 2>nul || echo [WARNING] voxcpm import failed
